 ...
 * Coming Soon, New Season [date], Premiere, New Episode, Season Finale [date], Series Finale.
 * Returns null (no pill) once the most recent episode is more than showCfg.recencyWindowDays
 ...
 * within showCfg.comingSoonWindowDays), gets its own pre-release heads-up -- e.g. The
 * Gentlemen season 2 dropping tomorrow. This ONLY fires before the season airs. Once a
 * season's air_date has passed, this function no longer calls it "New Season" at all -- it
 * falls straight through to the ordinary Premiere/New Episode/Finale logic below, same as any
 * other already-aired content. (An earlier version of this tag fired on a season that had
 * already aired, within the recency window -- so a still-recent season showed up as e.g. "New
 * Season Aug 27" days after the fact, a forward-looking label carrying a past date. Fixed:
 * New Season is now exclusively the pre-release heads-up, symmetric with how Coming Soon
 * works for a show's very first season.)
 *
 * Premiere is the season-1 version of "just aired": if season 1's own air_date is within
 * showCfg.recencyWindowDays in the past, that's a Premiere, checked independently of
 * last_episode_to_air. This matters for Netflix-style drops where a whole season releases the
 * same day -- last_episode_to_air would point at whichever episode aired last (e.g. episode 8
 * of an 8-episode season), not episode 1, so checking last_episode_to_air alone would misread
 * a same-day season drop as "New Episode" instead of "Premiere". Also naturally covers the
 * single-episode-season edge case (a TV movie, one-off special, or a show canceled after one
 * episode -- technically both the first and last episode of the last season, and should still
 * read "Premiere", not "Finale"). Seasons 2+ don't get an equivalent past-tense label here:
 * once a new season has actually aired, "New Episode"/"Season Finale" below (driven by
 * last_episode_to_air) already describe it accurately -- re-flagging episode 1 off the
 * season's own air_date once it's in the past would ignore whatever episode most recently
 * aired.
 *
...
function computeShowContext(details, showCfg) {
  ...
  if (recentPremiere) return 'Premiere';
  ...
  // Fallback for the day(s) right after a season 2+ premiere, before TMDB's own
  // `last_episode_to_air` field has caught up. That field is known to lag the real air
  // date by up to about a week right after a fresh episode/season drops (see the
  // Reacher case, Session 11) -- for a same-day season premiere specifically, it can
  // still be pointing at the *previous* season's finale from months back, which the
  // check above then (correctly, per its own logic) treats as too old and returns null
  // -- leaving the show tagless the moment its new season actually airs. This reads the
  // season's own `air_date` instead (the same field `upcomingSeason`/`recentPremiere`
  // above already trust) as a backstop: if some season > 1 started within the last
  // `recencyWindowDays`, that's still "New Episode" news even if `last_episode_to_air`
  // hasn't caught up yet. Deliberately labeled "New Episode", not "New Season" --
  // "New Season" is reserved for the pre-release heads-up above (Session 11 fix); this
  // only closes the gap for the day(s) the season actually drops.
  const recentSeasonDrop = seasons.find((s) => {
    if (s.season_number <= 1 || !s.air_date) return false;
    const d = daysSince(s.air_date);
    return d >= 0 && d <= showCfg.recencyWindowDays;
  });
  if (recentSeasonDrop) return 'New Episode';
