...
// Movies: "Just Added" for the first week after the confirmed US digital/physical release,
// "Now Streaming" for the week after that, no pill at all past 14 days -- a title that's been
// out for a month doesn't need a freshness callout every time it shows up in the list.
const MOVIE_JUST_ADDED_WINDOW_DAYS = 7;
const MOVIE_NOW_STREAMING_WINDOW_DAYS = 14;

// Shows: any episode-based tag (Premiere / New Season / New Episode / Season Finale /
...
async function collectUntilFilled(pathname, resolveItem, targetCount = TARGET_COUNT, maxPages = MAX_PAGES) {
  ...
  while (passing.length < targetCount && page <= totalPages && page <= maxPages) {
    ...
    const resolved = await Promise.all(pageResults.map(resolveItem));
    for (const item of resolved) {
      if (item) passing.push(item);
    }
    page += 1;
  }
...
/**
 * "Just Added" within a week of the confirmed US digital/physical release, "Now Streaming"
 * for the week after that, null (no pill) once it's been out more than two weeks.
 */
 ...
 * Premiere, New Episode, New Season, Season Finale, Finale <date>. Returns null (no pill)
 ...
 *
 * Premiere (season 1, episode 1) is checked first and unconditionally, ahead of the series-
 * finale check: a single-episode season 1 (a TV movie, one-off special, or a show canceled
 * after one episode) is technically both the first AND last episode of the last season, and
 * should still read "Premiere", not "Finale".
 */
function computeShowContext(details) {
  ...
  if (ep.season_number === 1 && ep.episode_number === 1) return 'Premiere';
