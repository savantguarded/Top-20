...
function computeMovieContext(digitalReleaseDate, physicalReleaseDate, upcomingDigitalReleaseDate, movieCfg) {
  ...
  // Coming Soon is only for titles with no digital release yet. A movie that already went
  // PVOD can later get a second US type-4 date (e.g. its HBO Max/streaming-service debut);
  // without this guard it'd be mislabelled Coming Soon in the days before that date.
...
// ---- Landscape art: textless backdrop + clearlogo -----------------------------------------
//
// Both come from the `images` block appended to the details call each title already makes
// (append_to_response), so no extra TMDB request. Why they matter (confirmed against Nuvio
// TV's own source, ModernHomeRows.kt): on a landscape card Nuvio draws the title's clearlogo
// OVER the card image. It takes that logo from the catalog item's own `logo` field if there is
// one, otherwise it waits for a per-item metadata fetch that only fires for items near focus --
// which is why logos used to appear only once a card (or its neighbour) was focused. Sending
// `logo` here makes it show on every card straight away. And because Nuvio always adds its own
// logo, the card image underneath must NOT have a title/logo baked in, or it doubles up -- hence
// preferring a textless backdrop (iso_639_1 === null) over TMDB's default backdrop_path, which
// is sometimes a text-bearing one.

/** Language list for include_image_language: English, the title's original language, textless. */
...
/** Textless backdrops, best-voted first; full-HD-ish ones (>=1280 wide) ahead of small ones. */
function textlessBackdrops(images) {
  const all = ((images && images.backdrops) || []).filter((b) => b.iso_639_1 == null && b.file_path);
  const big = all.filter((b) => !b.width || b.width >= 1280);
  return (big.length ? big : all).sort(byVotes);
}

/** Top-voted textless backdrop, or TMDB's default backdrop_path if there's no textless one. */
function pickTextlessBackdrop(images, fallbackPath) {
  const textless = textlessBackdrops(images);
  return textless.length ? textless[0].file_path : fallbackPath || null;
}

// How many top-voted alternates lib/art.js compares when choosing the most distinct one.
const ART_ALTERNATES = 5;

/**
 * Landscape art candidates for one title. api/catalog.js picks between them per the
 * /backstage "Landscape art" setting (cfg.landscapeArt):
 *  - backdrop_path: the landscape CARD image, a DIFFERENT image from the main backdrop.
 *    Starts as the best-voted textless alternate; getTopMovies/getTopShows then refine it with
 *    lib/art.js, which picks whichever of the top alternates looks least like the references.
 *  - main_backdrop_path: the clean, unbadged BACKDROP (top-voted textless, else TMDB's default),
 *    sent as the catalog's `background` so clients that use it as a full-screen backdrop
 *    (Wuplay, Nuvio's hero) get plain art instead of our edited card.
 *  - logo_backdrop_path: a TMDB backdrop with the title logo already in it ('tmdb-logo' mode).
 * References = TMDB's main backdrop_path and the top-voted textless one, since the
 * metadata addon behind the row could be showing either.
 */
/** Best TMDB backdrop that has the title logo baked in: English first, then the original
 * language. Used by landscapeArt mode 'tmdb-logo'; null when TMDB has none. */
function pickLogoBackdrop(images, originalLanguage) {
  const all = ((images && images.backdrops) || []).filter((b) => b.file_path && b.iso_639_1 != null);
  const big = all.filter((b) => !b.width || b.width >= 1280);
  const pool = big.length ? big : all;
  const rank = (b) => (b.iso_639_1 === 'en' ? 0 : b.iso_639_1 === originalLanguage ? 1 : 2);
  const best = pool.filter((b) => rank(b) < 2).sort((a, b) => rank(a) - rank(b) || byVotes(a, b))[0];
  return best ? best.file_path : null;
}

function landscapeArt(images, mainPath, originalLanguage) {
  const textless = textlessBackdrops(images);
  const topTextless = textless.length ? textless[0].file_path : null;
  const refs = [...new Set([mainPath, topTextless].filter(Boolean))];
  const alternates = textless.map((b) => b.file_path).filter((p) => !refs.includes(p)).slice(0, ART_ALTERNATES);
  return {
    backdrop_path: alternates[0] || topTextless || mainPath || null,
    main_backdrop_path: topTextless || mainPath || null,
    logo_backdrop_path: pickLogoBackdrop(images, originalLanguage),
    artReferences: refs,
...
function pickLogo(images, originalLanguage) {
  ...
  const rank = (l) => (l.iso_639_1 === 'en' ? 0 : l.iso_639_1 === originalLanguage ? 1 : 2);
...
async function resolveMovie(m, cfg) {
  ...
  try {
    const details = await tmdbGet(`/movie/${m.id}`, {
      append_to_response: 'external_ids,release_dates,images',
    ...
    return {
      ...
      ...landscapeArt(details.images, details.backdrop_path, m.original_language),
      logo_path: pickLogo(details.images, m.original_language),
...
async function resolveShow(s, cfg) {
  ...
  try {
    ...
    return {
      ...
      ...landscapeArt(details.images, details.backdrop_path, s.original_language),
      logo_path: pickLogo(details.images, s.original_language),
...
async function getTopMovies() {
  ...
  await applyDistinctBackdrops(items);
  return items;
}
