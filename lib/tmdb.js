...
//  - getTopShows(): TMDB's daily trending TV list, US region. Unlike movies, no release-date
//    eligibility filter -- trending/tv/day can surface a show ahead of its premiere on hype
//    alone, computeShowContext's Coming Soon check handles that instead of excluding it.
 ...
 * Coming Soon, Premiere, New Episode, New Season, Season Finale <date>, Series Finale <date>.
 ...
 * first_air_date, last_episode_to_air, seasons) -- no extra requests needed.
 *
 ...
 * which assumes the show has aired at least one episode.
 *
...
function computeShowContext(details) {
  if (details.first_air_date) {
    ...
    if (premiereDays < 0) return 'Coming Soon'; // hasn't aired its first episode yet
  }
  ...
  if (isSeasonFinaleEp && isLastSeason && showEnded) return `Series Finale ${formatShortDate(ep.air_date)}`;
