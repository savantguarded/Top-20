...
async function collectUntilFilled(pathname, resolveItem, targetCount, maxPages) {
  ...
  while (passing.length < targetCount && page <= totalPages && page <= maxPages) {
    ...
    // Process strictly sequentially, one by one. This is safest for TMDB limits.
    for (const raw of uniquePageResults) {
      try {
        const item = await resolveItem(raw);
        if (item && !seenImdbIds.has(item.imdbId)) {
          seenImdbIds.add(item.imdbId);
          passing.push(item);
        }
      } catch (err) {
        lastError = err.message;
        console.error(`[Item Error] ${raw.id}:`, err.message);
      }

      if (passing.length >= targetCount) break;
      
      // Mandatory delay to keep under TMDB's limit
      await sleep(150); 
    }
  ...
  // If TMDB rejected everything, crash loudly so it shows in the browser
  if (passing.length === 0 && lastError) {
    throw new Error(`All items dropped. Last error: ${lastError}`);
  }
