...
function pickTextlessBackdrop(images, fallbackPath) {
  const textless = ((images && images.backdrops) || []).filter((b) => b.iso_639_1 == null && b.file_path);
  if (!textless.length) return fallbackPath || null;
  return textless.sort(byVotes)[0].file_path;
}
...
async function resolveMovie(m, cfg) {
  ...
  try {
    ...
    return {
      ...
      backdrop_path: pickTextlessBackdrop(details.images, details.backdrop_path),
