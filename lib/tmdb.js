...
async function collectUntilFilled(pathname, resolveItem, targetCount, maxPages) {
  ...
  while (passing.length < targetCount && page <= totalPages && page <= maxPages) {
    ...
    // BATCH FETCHING: Process 10 items at a time to prevent Rate Limits AND Vercel Timeouts
    for (let i = 0; i < uniquePageResults.length; i += 10) {
      const chunk = uniquePageResults.slice(i, i + 10);
      const resolved = await Promise.all(chunk.map(raw => resolveItem(raw)));
...
async function verifyImdbMatch(tmdbId, imdbId, title, kind) {
  try {
    ...
    console.error(`[IMDb Verify Error] ${imdbId}:`, err.message);
    return false;
  }
}
...
async function resolveMovie(m, cfg) {
  ...
  try {
    ...
    console.error(`[Movie Error] Failed on ${m.title} (${m.id}):`, err.message);
    return null;
  }
}
...
async function resolveShow(s, cfg) {
  ...
  try {
    ...
    console.error(`[Show Error] Failed on ${s.name} (${s.id}):`, err.message);
    return null;
  }
}
