...
// Whole days from dateStr to today (UTC). Negative = future.
 ...
 *  - Premiere / Season Premiere: a season whose own air_date is within the recency window.
 *    Read off the season, not last_episode_to_air, because full-season drops point that at the
 *    last episode, and TMDB lags updating it for up to a week after release.
 ...
 *  - Series Finale / Season Finale / Airing Today / New Episode: latest episode within the
 *    window. Airing Today only on the air date itself (today's episode may still sit in
 *    next_episode_to_air while TMDB catches up), then it reverts to New Episode.
 */
function computeShowContext(details, showCfg) {
  ...
  const ep = [details.next_episode_to_air, details.last_episode_to_air].find(airsToday) || details.last_episode_to_air;
  if (ep && ep.air_date) {
    ...
    if (days >= 0 && days <= showCfg.recencyWindowDays) {
      ...
      return days === 0 ? 'Airing Today' : 'New Episode';
    }
  }
 ...
 * display_priority among `flatrate`), or null. Rent/buy stores are ignored. Data: JustWatch.
 */
function topStreamingLogo(watchProviders, region) {
  ...
  const top = ((r && r.flatrate) || []).filter((p) => p.logo_path)
