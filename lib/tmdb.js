...
/**
 * Landscape card art for one title. `mode` is cfg.landscapeArt:
 *  - 'main': the top-voted textless backdrop (same image the backdrop behind the row
 *    usually shows).
 *  - 'alternate' (default): a DIFFERENT image. backdrop_path starts as the best-voted
 *    textless backdrop that isn't the main one; getTopMovies/getTopShows then refine it with
 ...
 */
function landscapeArt(images, mainPath, mode) {
  const textless = textlessBackdrops(images);
  const topTextless = textless.length ? textless[0].file_path : null;
  if (mode === 'main') {
    return { backdrop_path: topTextless || mainPath || null, artReferences: [], artAlternates: [] };
  }
...
async function resolveMovie(m, cfg) {
  ...
  try {
    ...
    return {
      ...
      ...landscapeArt(details.images, details.backdrop_path, cfg.landscapeArt),
...
async function getTopMovies() {
  ...
  if (cfg.landscapeArt !== 'main') await applyDistinctBackdrops(items);
  return items;
}
