...
async function collectUntilFilled(pathname, resolveItem, targetCount, maxPages) {
  ...
  while (passing.length < targetCount && page <= totalPages && page <= maxPages) {
    ...
    // FETCH SEQUENTIALLY WITH DELAY TO AVOID TMDB RATE LIMITS
    const resolved = [];
    for (const raw of uniquePageResults) {
      resolved.push(await resolveItem(raw));
      await sleep(100); // 100ms delay between each title request
    }
