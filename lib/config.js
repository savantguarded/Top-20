...
const DEFAULTS = {
  ...
  // Base poster image source, as a URL template. `{imdbId}` (or `{id}`/`{imdb_id}` -- any
  // spelling containing "id" works, since third-party providers document their own
  // placeholder differently) is replaced with the title's IMDb id (e.g. "tt1234567"). Swap
  // providers by changing just this string -- no code change. TMDB's own poster is always
  // the automatic fallback if this URL 404s, errors, or takes longer than api/poster.js's
  // own timeout to answer.
  //
  // Providers keyed on TMDB's own id instead of (or in addition to) imdb -- e.g. Posters+ --
  // can also use `{tmdb_id}` (the numeric TMDB id, threaded through from api/catalog.js),
  // `{type}` (translated to TMDB's own "movie"/"tv", not Stremio's "movie"/"series"), and
  // `{tmdb_key}` (this server's own TMDB_API_KEY, filled in server-side only -- never exposed
  // to the Stremio/Nuvio client). See buildPosterUrl() in api/poster.js for the exact rules.
  //
  // Examples:
  //   btttr.cc (current default):
  //     "https://btttr.cc/poster-n/imdb/poster-default/{imdbId}.jpg?tag=none"
  //   Posters+ (self-hosted instance, TMDB-id-keyed, badges/ratings/sashes turned off so it
  //   doesn't double up with this addon's own rank badge + status pill):
  //     "https://postersplus.slokker.cc/poster?tmdb_id={tmdb_id}&type={type}&tmdb_key={tmdb_key}&..."
  ...
  posterUrlTemplate: 'https://btttr.cc/poster-n/imdb/poster-default/{imdbId}.jpg?tag=none',

  // Same idea as posterUrlTemplate, but for the landscape/backdrop image used by Nuvio's
  // hero carousel and any landscape-mode catalog card (the `background` field in
  // api/catalog.js). `{backdrop_path}` is TMDB's own backdrop path (already includes its
  // own leading slash, e.g. "/abc123.jpg") threaded through from api/catalog.js -- the
  // default template just points straight at TMDB's own image host, so this works with zero
  // setup. A TMDB-id-keyed provider (the same kind of thing Posters+ is for posters) can
  // also use `{tmdb_id}`, `{type}`, and `{tmdb_key}` -- see buildPosterUrl() in
  // api/poster.js for the exact substitution rules, shared with posterUrlTemplate above.
  // Falls back to TMDB's own backdrop directly if this URL 404s, errors, or times out.
  backdropUrlTemplate: 'https://image.tmdb.org/t/p/w1280{backdrop_path}',

  // Landscape card art (the catalog's `landscapePoster`). Every mode ships a finished card with
  // the logo already in it, so Nuvio never overlays its own clearlogo (it only does that when
  // `landscapePoster` is absent -- NuvioTV ModernHomeRows.kt):
  //  - 'tmdb-logo': TMDB's own backdrop with the title logo baked in, untouched. Falls back to
  //    'alternate' for titles where TMDB has no logo-bearing backdrop.
  //  - 'alternate' (default): a clean textless TMDB alternate, distinct from the main backdrop
  //    (lib/art.js), with the TMDB clearlogo drawn bottom-left by lib/badge.js.
  //  - 'custom': backdropUrlTemplate above (e.g. Posters+, which bakes in its own logo).
...
/**
 * Read the raw "topTwentyConfig" Edge Config item, unmerged with DEFAULTS (null if unset,
 * or Edge Config isn't configured, or the read fails). Unlike getConfig(), this is what
 * api/config.js needs to edit a single field (e.g. posterUrlTemplate) without clobbering
 * every other override that isn't shown on the simplified settings page -- getConfig()'s
 * return value already has every default filled in, so writing it back verbatim would
 * "freeze" today's defaults as explicit overrides forever, including for fields the page
 * no longer exposes a control for.
 */
...
module.exports = { getConfig, DEFAULTS, mergeDeep, primeCache, getRawOverrides };
