// lib/badge.js
// Three overlays composited onto a poster:
//  1. A glossy rank number inset in the top-left corner (unchanged from before).
//  2. A small rounded chip in the top-right corner holding the title's primary streaming
//     provider's logo (e.g. Netflix) -- mirrors the rank badge's inset on the other side.
//  3. A bottom-flush "liquid glass" status pill -- a blurred/tinted crop of the poster
...
//     fixed-width bar.
// Rendering pipeline for all three: SVG (gradients / blurred image embed / text) -> resvg-js
// rasterizer (bundled Inter font, no system fonts needed) -> sharp composite (plus a plain
// sharp resize/composite for the provider logo chip, since that one's just a raster image,
// no text or gradients to render).
...
const PROVIDER_CHIP_SIZE_RATIO = 0.16; // relative to poster width, square chip
const PROVIDER_CHIP_MARGIN_RATIO = 0.05; // inset from the top-right corner, relative to poster width
const PROVIDER_CHIP_CORNER_RATIO = 0.22; // relative to chip size
const PROVIDER_CHIP_PADDING_RATIO = 0.16; // logo padding inside the chip, relative to chip size
const PROVIDER_CHIP_TINT = '#000000';
// Solid enough to read the logo against any poster art without a blurred poster crop like
// the bottom pill uses -- the chip is small enough that a plain dark wash reads cleanly and
// it saves an extra sharp extract/blur pass for a corner element this size.
const PROVIDER_CHIP_TINT_OPACITY = 0.55;

function providerChipSvg(size) {
  const r = Math.round(size * PROVIDER_CHIP_CORNER_RATIO);
  return `
<svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}" xmlns="http://www.w3.org/2000/svg">
  <rect x="0" y="0" width="${size}" height="${size}" rx="${r}" ry="${r}" fill="${PROVIDER_CHIP_TINT}" fill-opacity="${PROVIDER_CHIP_TINT_OPACITY}"/>
  <rect x="0.5" y="0.5" width="${size - 1}" height="${size - 1}" rx="${r}" ry="${r}" fill="none" stroke="#ffffff" stroke-opacity="0.25" stroke-width="1"/>
</svg>`;
}

/**
 * Composite a provider's logo (e.g. Netflix) into a small rounded chip inset in the
 * poster's top-right corner, mirroring the rank badge's top-left inset. `logoBuffer` is
 * whatever image bytes the caller already fetched (poster.js pins the fetch to
 * image.tmdb.org), resized to fit the chip with padding. Returns a PNG buffer sized to
 * `size`, ready to composite onto the poster.
 */
async function renderProviderChip(logoBuffer, size) {
  const chipBg = renderSvgToPng(providerChipSvg(size), size);
  const padding = Math.round(size * PROVIDER_CHIP_PADDING_RATIO);
...
/**
 * Composite the rank badge, the provider logo chip, and/or the status pill onto a poster
 * image buffer. Pass rank as null/undefined to skip the rank badge, statusLabel as
 * null/undefined/empty to skip the pill, providerLogoBuffer as null/undefined to skip the
 * chip -- so callers can use any combination. Returns a JPEG buffer.
 */
async function applyOverlays(posterBuffer, { rank, statusLabel, providerLogoBuffer } = {}) {
  ...
  if (providerLogoBuffer) {
    const size = Math.round(w * PROVIDER_CHIP_SIZE_RATIO);
    const margin = Math.round(w * PROVIDER_CHIP_MARGIN_RATIO);
    try {
      const chipPng = await renderProviderChip(providerLogoBuffer, size);
      composites.push({ input: chipPng, left: w - margin - size, top: margin });
    } catch {
      // a corrupt/unsupported logo image shouldn't take down the whole poster render
    }
  }
...
module.exports = { applyOverlays, renderBadgePng, badgeSvg, pillSvg, pillRect, pillFontSize, renderProviderChip };
