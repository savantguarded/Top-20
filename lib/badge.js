...
// A true frosted-glass look: blur whatever poster is behind the pill and lay a neutral,
// mostly-see-through dark wash over it for text contrast, rather than tinting toward any
// one color. Neutral + fairly transparent so it reads well against a light poster, a dark
// poster, or anything in between, instead of only looking right against warm/dark art.
const PILL_TINT = '#000000';
const PILL_TINT_OPACITY = 0.38;
const PILL_BLUR_SIGMA = 10;

// Text sizing: every pill uses the SAME font size, so the row of tags looks consistent
// instead of each label being sized to its own length. That size is derived from the
// longest label the app actually produces -- "Now Streaming" / "Season Finale" / a
...
// ~80% fill). Shorter labels like "Premiere" just end up with more side margin, which is
// the intended look.
const PILL_TEXT_WIDTH_RATIO = 0.82; // target ink width, as a fraction of pill width, for the reference label
...
const PILL_FONT_MAX_RATIO = 0.62; // cap, relative to pill height
...
function pillFontSize(w, h) {
  ...
  const raw = targetTextWidth / (PILL_REFERENCE_CHAR_COUNT * AVG_CHAR_WIDTH_FACTOR);
