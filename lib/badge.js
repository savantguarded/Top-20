...
//     exposed bottom edge) -- see pillRect()/pillPath() below. Every other pill constant
//     (font size, letter-spacing, padding, colors, blur) is identical between shapes.
 ...
 */
function pillRect(label, posterW, posterH, edge) {
  ...
  const y = edge === 'top' ? 0 : posterH - h; // flush with the top or bottom edge
 ...
 */
async function applyOverlays(posterBuffer, { rank, statusLabel, corner = 'tl', shape = 'portrait' } = {}) {
  ...
  if (statusLabel) {
    const { x, y, w: pw, h: ph, fontSize, letterSpacing } = pillRect(statusLabel, w, h, pillEdge);
