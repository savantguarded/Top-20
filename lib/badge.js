 ...
 */
function badgeSvg(rank, posterW, posterH, corner) {
...
function renderBadgePng(rank, posterW, posterH, corner) {
  return renderSvgToPng(badgeSvg(rank, posterW, posterH, corner), posterW);
}
 ...
 * see badgeSvg() above. Returns a JPEG buffer.
 */
async function applyOverlays(posterBuffer, { rank, statusLabel, corner = 'tl' } = {}) {
  ...
  if (rank) {
    composites.push({ input: renderBadgePng(rank, w, h, corner), left: 0, top: 0 });
  }
