...
// (custom Inter Black font loaded from disk, no system fonts needed) -> sharp composite.
...
function badgeSvg(rank, posterW, posterH) {
  ...
  // every rank so digit count doesn't shift the badge's position.
  const x = posterW * 0.07;
  const y = posterH * 0.205;
