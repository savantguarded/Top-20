...
function badgeSvg(rank, posterW, posterH) {
  ...
  // so a "20" doesn't look cramped next to a "4".
  const fontSize = isDouble ? posterW * 0.40 : posterW * 0.52;

  // Anchor point: bleed off the top-left corner a bit, like the reference image.
  const x = posterW * (isDouble ? -0.005 : 0.015);
  const y = posterH * 0.34;
...
<svg width="${posterW}" height="${posterH}" viewBox="0 0 ${posterW} ${posterH}" xmlns="http://www.w3.org/2000/svg">
  <defs>
    ...
    <filter id="dropShadow" x="-60%" y="-60%" width="220%" height="220%">
      <feDropShadow dx="0" dy="${posterW * 0.012}" stdDeviation="${posterW * 0.018}" flood-color="#000000" flood-opacity="0.65"/>
      <feDropShadow dx="0" dy="0" stdDeviation="${posterW * 0.006}" flood-color="#000000" flood-opacity="0.55"/>
    </filter>
  </defs>
