...
// Text sizing: rather than a couple of hardcoded font sizes, size the label so it fills
// a consistent share of the pill width, then clamp so very short labels don't balloon and
// very long ones don't shrink to nothing. Calibrated against toptoday.llamayu.com's own
// rendering of "Just Added" (measured: ~189px of ink inside a 238px-wide pill, i.e. it
// fills about 80% of the pill) -- our previous fixed h*0.34 size was well under that,
// which is why it looked small.
const PILL_TEXT_WIDTH_RATIO = 0.82; // target ink width as a fraction of pill width
const AVG_CHAR_WIDTH_FACTOR = 0.53; // avg glyph advance as a fraction of font-size, Inter Bold
const PILL_FONT_MAX_RATIO = 0.62; // cap, relative to pill height, for very short labels
const PILL_FONT_MIN_RATIO = 0.2; // floor, relative to pill height, for very long labels
...
function pillFontSize(label, w, h) {
  const charCount = Math.max(String(label).length, 1);
  const targetTextWidth = w * PILL_TEXT_WIDTH_RATIO;
  const raw = targetTextWidth / (charCount * AVG_CHAR_WIDTH_FACTOR);
...
function pillSvg(label, w, h, blurredCropBase64) {
  ...
  const fontSize = pillFontSize(label, w, h);
