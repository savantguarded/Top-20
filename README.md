...
- **Top Movies Today** — top 20 US movies currently out digitally or on home release (TMDB `discover/movie`, `with_release_type=4|6`, region `US`), ranked by popularity. Theatrical-only movies are excluded.
...
- **"Digital or home release"**: TMDB release type `4` = Digital, `6` = Physical. If you also want to include limited theatrical re-releases or premiere dates, adjust `with_release_type` in `lib/tmdb.js`.
