...
- **Catalog metadata is deliberately minimal**: each entry is `id`, `type`, `name`, `poster`, `background`, `logo`, `releaseInfo` — no `description`/`genres`/`imdbRating`/`runtime`. An over-full catalog preview competes with a real metadata addon (e.g. aiometadata) instead of deferring to it, so those stay that addon's job on the detail page. `logo` is the one exception, see below.
- **`background`**: a landscape (16:9) render of the title's top-voted *textless* TMDB backdrop with the same rank badge and status pill as `poster`, laid out for a wide frame (pill flush to the top edge, 1.75x taller). Served through the same `/poster/...` endpoint with `shape=landscape`; the base image source is swappable from `/backstage` ("Landscape / backdrop provider URL template"). Used by Nuvio's landscape catalog cards and hero carousel. The base image must not have a title logo baked in, because Nuvio draws its own (next bullet); Posters+'s landscape mode always bakes one in, so it doubles up there.
- **`logo`**: the title's TMDB clearlogo (English, then original language, PNG preferred), taken from the same TMDB details call via `append_to_response=images`, so no extra API request. Confirmed against Nuvio TV's own source (`ModernHomeRows.kt`): landscape cards draw the logo over the card image, taking it from the catalog's `logo` field if present and otherwise only fetching one lazily for items near focus. Without this field, logos popped in only once a card or its neighbour was focused.
